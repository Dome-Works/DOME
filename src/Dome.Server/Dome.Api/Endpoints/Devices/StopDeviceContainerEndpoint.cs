...
public sealed class StopDeviceContainerEndpoint : Endpoint<DeviceContainerLifecycleRequest>
{
    private readonly IDeviceContainerCommandService _deviceContainerCommandService;

    public StopDeviceContainerEndpoint(IDeviceContainerCommandService deviceContainerCommandService)
    {
    ...
    {
        Post("/api/devices/{Name}/containers/{ContainerId}/stop");
        AllowAnonymous();
    }
    ...
    {
        var result = await _deviceContainerCommandService.StopAsync(
            req.Name,
            req.ContainerId,
            ct);
